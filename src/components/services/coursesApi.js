...
const BASE_URLS = 'http://api.wisey.app/api/v1/';
...
export async function getToken() {
  ...
  const TOKENS = await axios.get(`${BASE_URLS}auth/anonymous`, options);
  return TOKENS.token;
}

export async function getTrendsCourses() {
  const options = {
    headers: {
  ...
  const respons = await axios.get(`${BASE_URLS}core/preview-courses`, options);
  return respons;
}
